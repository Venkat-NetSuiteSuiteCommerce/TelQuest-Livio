...
/*****************************************************
    ...
    Pacejet Shipping Integration Workflow
*/
var bundleversion = '6.2.6';
...
var custscript_pacejet_defaultpackagenumber = 'custscript_pacejet_' + scripttype + '_spdefpackagenum';
...
var custscript_pacejet_openconnector_url = 'custscript_pacejet_oc_url_' + scripttype;
var custscript_pacejet_usewms = 'custscript_pacejet_' + scripttype + '_useebiz';
var custscript_pacejet_workstation = 'custscript_pacejet_' + scripttype + '_workstation';
...
function customizeUI_ItemFulfillmentBeforeLoad(type, form) {
    try {
        ...
        //Check for AutoShip Checkbox value 
        if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_autoimport') === 'T') {
            if (nlapiGetRecordId() == null)
                nlapiSetFieldValue('custbody_pacejet_autoimport', currentContext.getSetting('SCRIPT', 'custscript_pacejet_autoimport_default'), null, true);
            form.getField('custbody_pacejet_autoimport').setDisplayType('normal');
        } else {
            form.getField('custbody_pacejet_autoimport').setDisplayType('hidden');
        }
        ...
        if (type == 'view' && currentContext.getExecutionContext() == 'userinterface') {
            ...
            //Determine Ship Status and show button
            ...
            var shipStatus = currentRecord.getFieldValue('shipstatus');
            if (shipStatus != 'C') {
                ...
                if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_' + scripttype + '_show_freightquote') === 'T') {
                    ...
                    if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_' + scripttype + '_speedintegration') == 'T') {
                        ...
                        buttonScript = "pj_ButtonClick(this,'" + currentContext.getSetting('SCRIPT', 'custscript_pacejet_oc_url_' + scripttype) + "Method=FreightQuoteImport&SubMethod=ItemFulfillment&TransactionID=" + currentId + "&ExternalTransactionID=" + currentId + "', false, " + (defnamedwindow == 'T' ? 'true' : 'false') + ", '" + Location + "', " + currentId + ", false, '" + license + "')";
                    }
                ...
                if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_' + scripttype + '_show_ship') === 'T') {
                    ...
                    if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_' + scripttype + '_speedintegration') == 'T') {
                        ...
                        buttonScript = "pj_ButtonClick(this,'" + currentContext.getSetting('SCRIPT', 'custscript_pacejet_oc_url_' + scripttype) + "Method=ShipImport&SubMethod=ItemFulfillment&TransactionID=" + currentId + "&ExternalTransactionID=" + currentId + "', false, " + (defnamedwindow == 'T' ? 'true' : 'false') + ", '" + Location + "', " + currentId + ", false, '" + license + "')";
                    }
                ...
                if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_' + scripttype + '_show_push') === 'T') {
                    ...
                    if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_' + scripttype + '_speedintegration') == 'T') {
                        ...
                        buttonScript = "pj_ButtonClick(this,'" + currentContext.getSetting('SCRIPT', 'custscript_pacejet_oc_url_' + scripttype) + "Method=ShipImport&SubMethod=ItemFulfillment&Action=Push&TransactionID=" + currentId + "&ExternalTransactionID=" + currentId + "', false, " + (defnamedwindow == 'T' ? 'true' : 'false') + ", '" + Location + "', " + currentId + ", false, '" + license + "')";
                    }
                ...
                if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_' + scripttype + '_show_autoship') === 'T') {
                    ...
                    if (currentContext.getSetting('SCRIPT', 'custscript_pacejet_' + scripttype + '_speedintegration') == 'T') {
                        ...
                        buttonScript = "pj_ButtonClick(this,'" + currentContext.getSetting('SCRIPT', 'custscript_pacejet_oc_url_' + scripttype) + "Method=ShipImport&SubMethod=ItemFulfillment&Action=AutoShip&TransactionID=" + currentId + "&ExternalTransactionID=" + currentId + "', true, " + (defnamedwindow == 'T' ? 'true' : 'false') + ", '" + Location + "', " + currentId + ", true, '" + license + "')";
                    }
            ...
            currentContext.setSessionObject('custscript_pacejet_if_workstation', nlapiGetFieldValue('custbody_pacejet_workstation'));
        } else if ((type == 'edit' || type == 'create') && currentContext.getExecutionContext() == 'userinterface') {
            Location = getSetting(custscript_pacejet_location, deflocation);
            createWorkstationDropdown(form);
        }
...
function createWorkstationDropdown(form) {
    //set default to the current value if there is one, otherwise, choose user level script deployment parameter
    var defWorkstation = nlapiGetFieldValue('custbody_pacejet_workstation') != null ? nlapiGetFieldValue('custbody_pacejet_workstation') : String(currentContext.getSessionObject('custscript_pacejet_if_workstation'));

    var header = { "Content-Type": "application/json; charset=utf-8" };
    var URL = URLSetting.protocol + '://' + URLSetting.host + '/webservices/general.svc/getWorkstations/' + Location;
    var response = nlapiRequestURL(URL, null, header);
    try {

        var body = JSON.parse(response.getBody());
        var select = form.addField('custpage_pj_workstation', 'select', 'Pacejet Workstation');
        select.addSelectOption("", "Select");
        for (var x = 0; x < body.length; x++) {
            select.addSelectOption(body[x].WorkstationWorkstationID, body[x].WorkstationName, defWorkstation == body[x].WorkstationWorkstationID);
        }

    } catch (e) {
        nlapiLogExecution('ERROR', 'Create Workstation Dropdown: ' + URL, e.message);
    }
}
